    ...
    private void OnDrawGizmos()
    {
        ...
        foreach (var joint in trackingPoints)
        {
            ...
            foreach (var camPos in cameraPositions)
            {
                ...
                // Perform raycast and collect all hits
                ...
                {
                    // Log debug info
                    Debug.Log($"Raycast from {camPos} to {joint.position} hit {hits.Length} objects.");

                    // First hit should be the character (skip it if true)
                    Transform firstHitTransform = hits[0].transform;

                    if (trackingPoints.Contains(firstHitTransform))
                    {
                        // If the first hit is already the tracking point, it's visible
                        isVisible = true;
                    }
                    else if (hits.Length > 1)
                    {
                        // Second hit should be the tracking point for it to be visible
                        Transform secondHitTransform = hits[1].transform;
                        if (trackingPoints.Contains(secondHitTransform))
                        {
                            isVisible = true;
                        }
                    }
